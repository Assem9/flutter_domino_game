...
class UserCubit extends Cubit<UserStates> {
  ...
  void deleteUserAuthData(context)async{
    await UserFireBaseServices().updateUserStatus(status: 'offline').then((value){
      uId = null ;
  ...
  void userDeleteAccount()async{
    userSigningOut = true ;
    emit(UserSigningOut());
    await UserFireBaseServices().deleteUserDocument().then((value)async{
      uId = null ;
      await CacheHelper.clearData(key: 'uId');
      user = null ;
      showToast(message: 'Account is deleted successfully') ;
      await Future.delayed(const Duration(seconds: 2),(){
 ...
 ////////////////////////////// edit User Data /////////////////////////////////////
  ...
  void addMatchRequest()async{
    try{
      ...
      emit(MatchRequestCreated());
    }catch(e){
      debugPrint('$e');
      emit(MatchRequestCreatingError());
    }
  }
  ...
  void updateMatchRequest(MatchRequest request)async{
    try{
      await UserFireBaseServices().updateMatchRequestDoc(request);
      emit(MatchRequestUpdated());
    }catch(e){
      debugPrint('$e');
      emit(MatchRequestUpdatingError());
    }
  ...
  void joinRequest() {
    if(!joinedRequest!.isTeamMatch){
      ...
      updateMatchRequest(joinedRequest!);
    }else{
